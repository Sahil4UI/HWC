            ...
            <div className="flex flex-wrap gap-2 justify-center bg-black/40 p-2 rounded-2xl backdrop-blur-md border border-white/5">
                {PRACTICE_DATA.map(tech => (
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all font-bold tracking-wide relative overflow-hidden group ${activeTech.id === tech.id
                                        ...
                                        {topic.questions.map(q => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`group flex items-center justify-between w-full text-left px-3 py-2 rounded-md text-xs transition-all ${activeQuestion.id === q.id
                ...
                <div className="flex-1 flex flex-col relative">
                    ...
                    <div className="p-8 border-b border-white/5 bg-[#121215] shadow-sm z-10 flex-shrink-0">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border shadow-lg ${activeQuestion.difficulty === 'Easy' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-emerald-900/20' :
                    ...
                    {/* Status Bar */}
                    <div className={`absolute bottom-6 right-6 px-6 py-3 rounded-xl shadow-2xl border flex items-center gap-3 transition-all duration-500 transform ${status === 'SUCCESS' ? 'translate-y-0 opacity-100 bg-emerald-500 text-black border-emerald-400' :
